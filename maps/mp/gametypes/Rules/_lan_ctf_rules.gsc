Rules()
{
	...
	setcvar("scr_ctf_roundlength" , "1");
	setcvar("scr_ctf_startrounddelay" , "5");
	...
	// Hostname and MOTD 
	setcvar("sv_hostname" , "Unknown Lan Match - Change Me"); 
	setcvar("scr_motd" , "LAN - Name Me!!"); 
	...
	if (getcvarint("g_ot_active") == 1)
	{
		...
		setcvar("scr_ctf_roundlength" , "2"); // Length of each round
...
SuddenDeathRules()
{
	level.flagtimeout = 20;	 //Changing Timing on Flag Timeout to Return to base on its own... -1 = Never
...
Set_Respawn_Time()
{
	...
	switch(level.mapname)
	{
		...
		case "mp_ponyri":
		case "mp_sicily":
			setcvar("scr_death_wait_time", "15");
			break;
