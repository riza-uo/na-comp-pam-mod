...
Rules()
{
	...
	level.matchscore = getcvarint("scr_sd_end_score");
	...
	{
		if(level.roundlimit == 0 && level.scorelimit == 0)
		{
			level.scorelimit = 7; // SET A DEFAULT SCORELIMIT HERE!
			setcvar("scr_sd_sd_scorelimit", level.scorelimit);
		}

		if (level.roundlimit != 0)
			setcvar("scr_sd_end_round", level.roundlimit);

		if (level.scorelimit != 0)
			setcvar("scr_sd_end_score", level.scorelimit);
	}
